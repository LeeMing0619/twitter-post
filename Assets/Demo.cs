...
public class Demo : MonoBehaviour 
{
    ...
    public RawImage img;
    string media_string;
    public InputField filepath_input;
    public Text result_lbl;
    ...
    {
        result_lbl.text = "";
        if (!string.IsNullOrEmpty(filepath_input.text))
        {
            filePath = filepath_input.text;
            processedTexture = LoadPNG();
            print("file is" + processedTexture);
            img.GetComponent<RawImage>().texture = processedTexture;
        }
    ...
    void OnGUI()
    {
        ...
        {
            StartCoroutine(Twitter.API.PostTweet(m_Tweet, media_string, CONSUMER_KEY, CONSUMER_SECRET, m_AccessTokenResponse,
    ...
    void OnPostTweet(bool success)
    {
        ...
        result_lbl.text = "OnPostTweet - " + (success ? "succedded." : "failed.");
    }
    string filePath;
    public Texture2D LoadPNG()
    {
        ...
        #if UNITY_EDITOR
                filePath = UnityEditor.EditorUtility.OpenFilePanel("Overwrite with Jpg, Png, bmp", "", "*.*");
        #endif

        if (File.Exists(filePath))
        {
            print(filePath);
            media_string = System.Convert.ToBase64String(File.ReadAllBytes(filePath));
